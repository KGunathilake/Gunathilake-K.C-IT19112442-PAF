...
$(document).on("click", ".btnRemove", function(event){
	...
	console.log(reqid);
		$.ajax({
			url : "ConRequestApi?requestId="+reqid,
			type : "Delete",
			dataType : "text",
			complete : function(response, status)
			{
...
$("#contact").val($(this).closest("tr").find('td:eq(7)').text());
$("#city").val($(this).closest("tr").find('td:eq(5)').text());
$("#postalCode").val($(this).closest("tr").find('td:eq(6)').text());
$("#message").val($(this).closest("tr").find('td:eq(8)').text());
$("#district").val($(this).closest("tr").find('td:eq(9)').text());
$("#contype").val($(this).closest("tr").find('td:eq(10)').text());
...
function validateItemForm()
	...
	if (!$.isNumeric(contact))
	{
		return "Insert a numerical value for Postal Code";
	}
